...
enum AppUpdater {
    ...
    static func check(manual: Bool) {
        ...
        Task { @MainActor in
            ...
            guard isNewer(latest.version, than: current) else {
                if manual { alert("You’re up to date", "Amped \(current) is the latest version.") }
                return
            }
    ...
    private static func offer(version: String, page: URL) {
        let panel = NSAlert()
        panel.messageText = "Amped \(version) is available"
        panel.informativeText = "Update with `brew upgrade --cask amped`, or download it from GitHub."
